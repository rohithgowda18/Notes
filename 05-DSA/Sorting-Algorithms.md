...
2. [Bubble Sort (Adjacent Compare & Swap)](#2-bubble-sort-adjacent-compare--swap)
3. [Selection Sort (Find Minimum & Place at Front)](#3-selection-sort-find-minimum--place-at-front)
...
Before diving into specific algorithms, understand the 2 fundamental properties frequently asked in interviews:
...
|                   | In-Place   : Bubble Sort, Selection Sort, Insertion Sort, Quick Sort (O(log n) stack)|
...
## 2. Bubble Sort (Adjacent Compare & Swap)
...
* **Pass 1**:
  * Compare `(5, 3)` $ightarrow 5 > 3 ightarrow$ Swap $ightarrow `[3, 5, 8, 1]`$
  * Compare `(5, 8)` $ightarrow 5 < 8 ightarrow$ No swap $ightarrow `[3, 5, 8, 1]`$
  * Compare `(8, 1)` $ightarrow 8 > 1 ightarrow$ Swap $ightarrow `[3, 5, 1, 8]`$ *(8 is locked at index 3)*
* **Pass 2**:
  * Compare `(3, 5)` $ightarrow$ No swap $ightarrow `[3, 5, 1, 8]`$
  * Compare `(5, 1)` $ightarrow 5 > 1 ightarrow$ Swap $ightarrow `[3, 1, 5, 8]`$ *(5 is locked at index 2)*
* **Pass 3**:
  * Compare `(3, 1)` $ightarrow 3 > 1 ightarrow$ Swap $ightarrow `[1, 3, 5, 8]`$ *(Array is sorted!)*

### ☕ Optimized Java Implementation (Early Stopping)
```java
...
## 3. Selection Sort (Find Minimum & Place at Front)
...
### 🔍 Trace Walkthrough `[5, 3, 8, 1]`
* **Pass 1**:
  * Scan `[5, 3, 8, 1]` $ightarrow$ Min is `1` at index 3.
  * Swap `arr[0]` (5) with `arr[3]` (1) $ightarrow `[1, 3, 8, 5]`$ *(1 is locked at index 0)*
* **Pass 2**:
  * Scan `[3, 8, 5]` $ightarrow$ Min is `3` at index 1.
  * Swap `arr[1]` with `arr[1]` $ightarrow `[1, 3, 8, 5]`$ *(3 is locked at index 1)*
* **Pass 3**:
  * Scan `[8, 5]` $ightarrow$ Min is `5` at index 3.
  * Swap `arr[2]` (8) with `arr[3]` (5) $ightarrow `[1, 3, 5, 8]`$ *(Sorted!)*
...
* **Auxiliary Space**: $\mathcal{O}(1)$ *(In-place)*
* **Stable**: **No (❌)** *(Long-distance swaps jump over equal elements, e.g. `[4a, 4b, 1]` $ightarrow$ 1 swaps with 4a, putting 4a behind 4b)*
...
* **Insert `3`**: Compare with 5 $ightarrow 5 > 3 ightarrow$ Shift 5 right $ightarrow$ Place 3 $ightarrow `[3, 5 | 8, 1]`$
* **Insert `8`**: Compare with 5 $ightarrow 8 > 5 ightarrow$ No shift $ightarrow `[3, 5, 8 | 1]`$
* **Insert `1`**: Compare with 8, 5, 3 $ightarrow$ Shift 8, 5, 3 right $ightarrow$ Place 1 $ightarrow `[1, 3, 5, 8]`$
...
> 🎯 **Placement High-Yield Tip**:  
> Insertion Sort is the algorithm of choice for **nearly-sorted / small arrays** ($n \le 16$) because of minimal constant overhead and $\mathcal{O}(n)$ best-case behavior (often used as the base case in hybrid algorithms like TimSort).
...
![Merge Sort Tree](images/merge-sort-tree.svg)
...
![Quick Sort Partition Flow](images/quick-sort-partition.svg)

```mermaid
flowchart TD
    A["[5, 3, 8, 1]"] --> B["1. Choose Pivot = 1 (last element)"]
    B --> C["2. Lomuto Partition"]
    C --> D["Left Subarray: []"]
    C --> E["Pivot Locked: [1]"]
    C --> F["Right Subarray: [5, 3, 8]"]
    F --> G["Recurse on Right Subarray"]
    G --> H["Final Result: [1, 3, 5, 8]"]
```
...
* **Bubble Sort**: *"Compare neighbours, bubble biggest to the right."*
* **Selection Sort**: *"Select minimum from unsorted, swap with current index."*
* **Insertion Sort**: *"Card deck sort — best for nearly sorted data ($\mathcal{O}(n)$)."*
* **Merge Sort**: *"Guaranteed $\mathcal{O}(n \log n)$ always, but requires $\mathcal{O}(n)$ extra memory."*
* **Quick Sort**: *"Fastest in practice with $\mathcal{O}(1)$ extra array space, but $\mathcal{O}(n^2)$ worst case without randomized pivot."*
...
> **Answer**:  
> 1. **Linked Lists**: Inserting and merging nodes requires $\mathcal{O}(1)$ pointer changes (no extra $\mathcal{O}(n)$ temporary buffer needed!). Linked lists have slow random access, which hurts Quick Sort's partitioning.  
> 2. **Arrays**: Arrays have $\mathcal{O}(1)$ random indexing and excellent CPU cache locality, making Quick Sort significantly faster in practice with zero extra allocation.
...
> 3. **Dual-Pivot / Introsort**: Switch to HeapSort if recursion depth exceeds $2 \log n$ (used in `Arrays.sort()` for primitives).
